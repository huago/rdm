...
class Cola_Com_Curl {
    ...
    private $writeLogRate = null;
    private $logUserData = array();
    private $url = null;

    /*
      public function __construct($timeout = 3, $connectTimeOut = 1)
      {
    ...
    public function __construct($arrParam) {
        ...
        $this->curl = curl_init();
        $this->product = isset($arrParam['product']) ? $arrParam['product'] : 'com';
        $this->writeLogRate = isset($arrParam['writeLogRate']) ? $arrParam['writeLogRate'] : 0.01;
        $this->logUserData = isset($arrParam['logUserData']) ? $arrParam['logUserData'] : array();
    }
    ...
    public function exec($_ch = null) {
        ...
        //srand(time());
        if ($this->writeLogRate > 1) {
            $this->writeLogRate = 1;
        }

        $randNum = rand(1, 10000);
        if ($randNum < intval($this->writeLogRate * 10000)) {
            $endTime = $this->microtime_float();
            $costTime = round($endTime - $startTime, 3) * 1000; //ms
            $arrData = array(
                'cost' => $costTime,
                'api' => empty($ch->url) ? curl_getinfo($ch->curl, CURLINFO_EFFECTIVE_URL) : $ch->url,
            );
            if (!empty($this->logUserData) && is_array($this->logUserData)) {
                $arrData += $this->logUserData;
            }
            Cola_Com_SyslogngExt::log('http request', $arrData, Cola_Com_SyslogngExt::LOG_TYPE_HTTP, $ch->product);
        }
